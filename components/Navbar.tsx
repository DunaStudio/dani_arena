  ...
  return (
    <div className="sticky top-0 z-50">
      <nav className="w-full bg-white px-6 md:px-20 py-4 flex items-center justify-between border-b border-porcelain">
        ...
        {/* Desktop links */}
        <div className="hidden md:flex items-center gap-5 xl:gap-9">
          {links.map((link) => (
            <Link
              key={link.label}
              href={link.href}
        ...
        <a
          href="#contacto"
          className="hidden md:block bg-charcoal text-white text-sm rounded-full py-3 px-7 hover:bg-goldenOrange transition-colors whitespace-nowrap"
        >
          Contacto
        </a>
      ...
      {/* Mobile menu — directly below nav, no offset math needed */}
      <div
        className={`md:hidden bg-white border-b border-porcelain overflow-hidden transition-all duration-300 ease-in-out ${open ? "max-h-screen py-6" : "max-h-0"}`}
      >
